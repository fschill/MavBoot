...
volatile can_stream_data_t *CAN_stream_tx[2], *CAN_stream_rx[2];
...
uint8_t CAN_get(can_stream_data_t *data, uint8_t ch);
...
static void inline can_receive_handler(can_channel_t channel, uint8_t handle, volatile can_stream_data_t *can_stream) {
	U8 i;
	...
	if (can_stream!=NULL) {
		for (i=0; i<can_stream->can_mob.dlc; i++) {
			buffer_put_lossy((buffer_t*)&can_stream->byte_buffer, can_stream->can_msg.data.u8[i]);
		}
		if (can_stream->packet_callback!=NULL) {
			can_stream->packet_callback((can_bus_mob_t*)&can_stream->can_mob);
		}
	}	
...
static void can0_tx_handler(void)
{
	...
	if (handle != 0x20)
	{
		...
		CAN_stream_tx[0]->transmission_active=false;
	}
}
...
__interrupt
#endif
static void can0_busoff_handler(void)
{
...
__interrupt
#endif
static void can0_cerr_handler(void)
{
	CANIF_clr_interrupt_status(0);
	CANIF_mob_disable(0, CAN_TX_MOB);   // switch off (no retransmission attempt)
...
__interrupt
#endif
static void can0_wakeup_handler(void)
{
	CANIF_clr_interrupt_status(0);
	CAN_stream_tx[0]->transmission_active=false;
...
static void can1_tx_handler(void)
{
	...
	if (handle != 0x20)
	{
		...
		CAN_stream_tx[1]->transmission_active=false;
	}
}
...
__interrupt
#endif
static void can1_busoff_handler(void)
{
...
__interrupt
#endif
static void can1_cerr_handler(void)
{

	CANIF_clr_interrupt_status(1);
	CANIF_mob_disable(1, CAN_TX_MOB);   // switch off (no retransmission attempt)
	CAN_stream_tx[1]->transmission_active=false;
...
__interrupt
#endif
static void can1_wakeup_handler(void)
{
	CANIF_clr_interrupt_status(1);
	CAN_stream_tx[1]->transmission_active=false;
...
void can_bus_register_packet_receive_callback(can_stream_data_t *data, can_callback_t *callback) {
	data->packet_callback=(void (*)(can_bus_mob_t *msg_mob)) callback;
}
